...
5. Update doc comments: `onvif_server.rs:57-61` (the `DEFAULT_FIRMWARE`/`DEFAULT_SERIAL` comments that say "tracked in `DEBT.md`") and the `OnvifConfig` field docs to describe the live-identity-preferred, config-fallback behaviour. Remove the "tracked in `DEBT.md`" phrasing (DEBT.md is being deleted in step 08).
