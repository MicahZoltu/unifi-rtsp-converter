...
pub struct Frame {
    ...
    /// 32-bit FLV tag timestamp in milliseconds. RTP converts it to the 90 kHz clock as `timestamp_ms * 90`.
    pub timestamp_ms: u32,
    /// NALUs in this frame in stream order, each without its start code or length prefix — ready for RTP single-NALU / FU-A packetization.
...
/// Opaque, comparable session id assigned to each registered client. Cheap to copy; used as the handle for `StreamState::remove_client`.
...
/// One registered RTSP client: its session id and the sending half of its bounded frame channel. Held inside the hub's lock; the matching `Receiver` is returned to the client thread by `add_client`.
struct ClientHandle {
    id: ClientId,
...
/// Shared in-memory hub the camera pipeline writes to and RTSP clients read from. Cloning a `StreamState` is cheap (one `Arc` bump) and yields another handle to the same hub, so the camera thread and each RTSP session thread can own a clone.
#[derive(Clone)]
...
impl StreamState {
    ...
    /// Removes the client with the given session id, if registered. No-op for an unknown id — e.g. a client already dropped by `publish_frame` or never registered. Returns `true` iff a client was actually removed.
    ...
    /// Number of clients currently registered. Intended for diagnostics and tests; not used in the hot path.
...
/// Acquires the hub lock, recovering the guard even if a previous holder panicked and poisoned the mutex. Poisoning does not invalidate the hub's owned data — every field remains structurally sound — so serving continues rather than propagating a poison panic to the camera or RTSP threads. The hub's own operations never panic (no indexing, no unwrap), so poisoning is only reachable if a future caller's `Drop` impl panics while a guard is held.
