//! RTSP server: the text-protocol half (step 10) and the runtime half
//! (step 11). The protocol half parses/builds RTSP requests and responses,
//! handles the five mandatory methods (OPTIONS, DESCRIBE, SETUP, PLAY,
//! TEARDOWN), and negotiates TCP-interleaved vs UDP RTP transports — pure
//! string/byte logic with no sockets. The runtime half (`RtspServer`,
//! `handle_client`, the per-session RTP pump, and the `PacketSink` test
//! seam) drives that protocol over a real `TcpListener`, feeds it frames
//! from `StreamState`, and ships RTP via `RtpPacketizer`.
//!
//! The session registry (`RtspSessions`) is owned per connection: RTSP
//! sessions do not span TCP connections, so each `handle_client` thread
//! holds its own registry and there is no cross-connection shared mutable
//! session state. Codec parameters for DESCRIBE are pulled from the shared
//! `StreamState` (cloned cheaply into each connection thread), which is the
//! point where the camera pipeline (step 12) meets the RTSP layer —
//! matching the boundary drawn in `PROJECT.md`.
...
/// Relaxed ordering suffices for the shutdown flag and client counter: they
/// are advisory signals, not synchronization that establishes happens-before
/// for other data (the `StreamState` mutex carries that burden).
const RELAXED: Ordering = Ordering::Relaxed;

/// Poll interval for the non-blocking accept loop, so the `shutdown` flag is
/// checked promptly rather than blocking until the next connection.
const ACCEPT_POLL_MS: u64 = 50;

/// Per-connection read timeout. The control read loop blocks on `read` for at
/// most this long before returning `TimedOut`, which lets the loop re-check
/// the `shutdown` flag and tear down an idle connection promptly on stop.
/// A playing-but-silent session is unaffected: the pump streams on the writer
/// while the control loop spins cheaply on read timeouts.
const READ_TIMEOUT_MS: u64 = 500;

/// Per-connection write timeout, bounding how long the shared send mutex can
/// be held across a `write_all`. A stuck client that stops draining its TCP
/// receive buffer yields a write error after this, tearing the session down
/// rather than blocking the pump or control thread indefinitely.
const WRITE_TIMEOUT_MS: u64 = 5_000;

/// Size of the per-read scratch buffer feeding the request framer.
...
/// otherwise grow the buffer unbounded; exceeding this closes the
/// connection. Named per the resource-bounds quality gate.
...
/// connection, per RFC 2326 §12.39 and `PROJECT.md` → "TCP Interleaved RTP".
const INTERLEAVED_FRAME_MARKER: u8 = 0x24;

/// Number of framing bytes preceding an interleaved RTP packet:
/// `[$][channel][len_hi][len_lo]`, per RFC 2326 §12.39.
const INTERLEAVED_FRAMING_BYTES: usize = 4;

/// Pump channel poll interval, so the `shutdown` flag is checked promptly
/// between frames rather than blocking indefinitely on `recv`.
const PUMP_POLL_TIMEOUT_MS: u64 = 200;

/// Multiplier and increment of the tiny splitmix64-style mixer used to derive
/// per-session SSRC / sequence-number seeds from wall-clock nanos and a
/// process-wide counter, avoiding a crate dependency. Values from Knuth's
/// MMIX constants.
const SPLITMIX_MULTIPLIER: u64 = 6_364_136_223_846_793_005;
const SPLITMIX_INCREMENT: u64 = 14_426_950_408_889_634_077;

/// 64-bit golden-ratio fractional constant (`(√5−1)/2`), used to mix the
/// process counter into the session seed so successive sessions diverge even
...
impl RtspServer {
    /// Creates a server that will bind `0.0.0.0:rtsp_port` for RTSP clients
    /// and advertise `server_ip` in SDP origins. `state` is the shared hub
    /// the camera pipeline publishes frames and codec parameters to.
    ...
    /// Binds the RTSP listener on `0.0.0.0:rtsp_port` and runs the accept
    /// loop until `shutdown()` is called. Each accepted connection is handled
    /// on its own thread.
...
/// One RTSP session's runtime bookkeeping, local to a single connection.
/// `receiver` is `Some` between SETUP and PLAY; the pump takes it on PLAY.
/// `udp_sock` is the server-side RTP socket bound at SETUP for UDP
/// transports (held here so a PLAY can hand it to the pump without
/// rebinding).
...
/// flag is set, or a write fails; in every case registered clients are
/// removed from the hub so their pumps drain and exit.
fn handle_client(
    stream: TcpStream,
    state: StreamState,
    server_ip: String,
        ...
        loop {
            ...
            match parsed {
                Ok(Some((req, consumed))) => {
                    ...
                    let mut resp =
                        handle_request(&req, &mut ctx.sessions, &ctx.server_ip, codec.as_ref());
...
/// Per-connection mutable state: the shared hub handle, the connection's
/// send mutex and peer address, the per-connection session registry, and the
/// `StreamState` client registrations created by SETUP. Grouping these into
/// one struct keeps the wiring methods focused and avoids passing a long
/// chain of borrows through every helper.
...
impl ConnectionCtx {
    /// Applies the side effects that follow a dispatched request: registering
    /// a `StreamState` client at SETUP, spawning the RTP pump at PLAY, and
    /// tearing one down at TEARDOWN. May replace `resp` (e.g. with `503` if a
    /// UDP server socket could not be bound at SETUP).
    ...
    /// SETUP wiring: registers a `StreamState` client and (for UDP) binds the
    /// server RTP socket. On UDP bind failure the session is rolled back and
    /// the response becomes `503 Service Unavailable`.
    ...
    /// PLAY wiring: takes the session's buffered receiver, builds the matching
    /// `PacketSink`, and spawns the per-session RTP pump.
    fn wire_play(&mut self, req: &RtspRequest) {
        ...
        let sink = match &reg.transport {
            Transport::Interleaved { rtp_ch, .. } => {
                Box::new(TcpInterleavedSink::new(self.writer.clone(), *rtp_ch))
                    as Box<dyn PacketSink + Send>
            }
            Transport::Udp { client_rtp, .. } => {
                ...
                Box::new(UdpSink::new(
                    sock,
                    SocketAddr::new(self.peer.ip(), *client_rtp),
                )) as Box<dyn PacketSink + Send>
            }
        };
    ...
    /// Removes every still-registered client for this connection from the hub,
    /// dropping their senders so any running pumps drain and exit.
    fn cleanup(&mut self) {
...
/// receiver, packetizes each per RFC 6184, and sends every RTP packet
/// through the sink. Exits on channel disconnect (TEARDOWN / client gone),
/// a sink write error (broken pipe), or the shutdown flag, then removes the
/// client from the hub so the camera thread never blocks on a dead session.
fn run_pump(
    receiver: Receiver<Frame>,
    mut sink: Box<dyn PacketSink + Send>,
    state: StreamState,
    client_id: ClientId,
    shutdown: Arc<AtomicBool>,
) {
...
/// Sink abstraction letting the pump send RTP packets to a real socket in
/// production and an in-memory `Vec` in tests, sharing one code path. The
/// `Send` bound is required so a boxed sink can move into the pump thread.
pub trait PacketSink {
    /// Sends one complete RTP packet. An error signals a broken transport
    /// (e.g. peer closed); the pump treats it as terminal.
    fn send(&mut self, pkt: &[u8]) -> io::Result<()>;
}

/// `PacketSink` writing RTP as `$`-framed interleaved data on the RTSP TCP
/// connection, per RFC 2326 §12.39. Shares the connection's send mutex with
/// control-response writes so bytes never interleave.
...
impl TcpInterleavedSink {
    /// Wraps the shared connection writer and the negotiated RTP channel id.
    ...
    fn send(&mut self, pkt: &[u8]) -> io::Result<()> {
        ...
        let len = u16::try_from(pkt.len()).map_err(|_| {
            io::Error::new(
                ...
                "RTP packet exceeds 65535 bytes",
            )
        })?;
...
/// `PacketSink` sending RTP as UDP datagrams to the client's negotiated RTP
/// port, per RFC 2326 §12.39 (`client_port`). The socket is bound to the
/// server RTP port advertised in SETUP so the client can correlate source
/// and advertised ports.
pub struct UdpSink {
    sock: UdpSocket,
    dst: SocketAddr,
}
...
impl UdpSink {
    /// Wraps a bound server RTP socket and the client `(ip, client_rtp)`
    /// destination.
...
/// In-memory `PacketSink` recording every packet for assertion in tests.
/// Shares the exact pump code path with the production sinks.
pub struct VecSink {
    packets: Vec<Vec<u8>>,
}
...
impl VecSink {
    ...
    /// Returns the recorded packets in send order.
    pub fn packets(&self) -> &[Vec<u8>] {
        &self.packets
    }

    /// Consumes the sink and returns the recorded packets in send order.
...
/// Drives one frame through the pump core: packetizes `frame` and sends every
/// resulting RTP packet via `sink`, advancing `packetizer`'s sequence number.
/// This is the single shared send path — `run_pump` calls it for each live
/// frame over a real `TcpInterleavedSink` / `UdpSink`, and tests call it over
/// a `VecSink` to assert byte-for-byte parity with
/// `RtpPacketizer::packetize_frame`. A send error (broken transport)
/// propagates as `Err` so the caller can tear the session down.
pub fn pump_frame_into(
...
/// Derives a per-session SSRC from wall-clock nanos xor'd with a
/// process-wide counter, then mixed via splitmix64. Avoids a randomness
/// crate; uniqueness across sessions is provided by the counter, not by
/// cryptographic strength (RTP SSRCs need only be locally unique, per
/// RFC 3550 §3).
fn random_ssrc() -> u32 {
    (splitmix64(session_seed()) >> 32) as u32
}

/// Derives a per-session initial RTP sequence number from the same seed,
/// per RFC 3550 §5.1's recommendation to start at a random offset.
fn random_seq() -> u16 {
    (splitmix64(session_seed()) >> 16) as u16
}

/// Per-call entropy: wall-clock nanos since the Unix epoch xor'd with a
/// monotonic process counter. `unwrap_or_default` keeps it panic-free if the
...
/// One splitmix64 round (Knuth MMIX), used to diffuse `session_seed`'s raw
/// entropy across the 32-bit SSRC and 16-bit sequence fields.
