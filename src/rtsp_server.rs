...
// Re-export the pure protocol surface at the `rtsp_server::` path so `main.rs` and the integration tests (`tests/rtsp_protocol.rs`, `tests/rtsp_server.rs`) keep resolving without changing their imports. The list is exactly the protocol items those callers import (the method handlers, `parse_request`, the request/response/session/transport types) plus `handle_request` (the dispatcher the integration test drives end-to-end) and `RtspError` (the error type of `parse_request`); runtime-internal helpers (`response`, `session_id`, the status codes, `SESSION_TIMEOUT_SECS`) are pulled in by the private `use` below instead of re-exported, so they do not widen the public surface.
pub use crate::rtsp_protocol::{handle_describe, handle_options, handle_play, handle_request, handle_setup, handle_teardown, parse_request, Method, RtspError, RtspRequest, RtspResponse, RtspSessions, Transport};
use crate::rtsp_protocol::{response, session_id, SESSION_TIMEOUT_SECS, STATUS_BAD_REQUEST, STATUS_OK, STATUS_SERVICE_UNAVAILABLE, STATUS_UNSUPPORTED_TRANSPORT};
