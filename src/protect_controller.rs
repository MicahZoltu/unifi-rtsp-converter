...
//!
//! The camera's UniFi keepalive is a WS **Ping** control frame (opcode `0x9`) carrying the text payload `ping-<N>`, and it must be answered with a WS **Text** frame `pong-<N>` — *not* a WS Pong control frame. `ws::WsConnection::read_frame` auto-replies to a Ping with a Pong and swallows the Ping, which would both answer incorrectly and hide the keepalive from this layer. The lower-level `pub` `ws::parse_frame` / `ws::encode_frame` functions are the intended escape hatch and give this session full control of control-frame handling, so `AvClientSession` calls them directly instead of owning a `WsConnection`: growing a "surface Pings / custom-pong" mode onto `WsConnection` would push one protocol's keepalive quirk into the general framing layer for no other caller's benefit.
...
/// One observed frame for diagnostic tracing. `AvClientSession::with_tracer` installs a callback that receives one of these for every frame read from or written to the wire, so a caller can hex-dump / log the exact AVClient exchange without touching the session's dispatch logic. Production code passes `None` (no overhead).
#[derive(Debug, Clone)]
...
/// `from` field the controller advertises in its replies. The camera addresses its messages `to: "UniFiVideo"`, so the controller's `from` is the same token.
const CONTROLLER_FROM: &str = "UniFiVideo";

/// `to` field the controller addresses its replies to — the camera's own `from` token.
...
/// `hello` reply payload fields. The real Protect controller's `hello` reply carries the controller identity (`controllerName`/`controllerUuid`/`controllerVersion`) plus an echoed `protocolVersion` and `overrideUuid: true` — NOT a `features` map (`service.js` `ubntAvclientHello`: `t.respond(r, {protocolVersion: g.protocolVersion, controllerName, controllerUuid, controllerVersion, overrideUuid: true}, false)`).
...
/// Video codec label the controller advertises on each `ChangeVideoSettings` video channel (`type` field). Confirmed against the Protect 7.1.77 source: `service.js` uses `VideoCodecLabel.H264 = "h264"`. The proxy only supports H.264 (audio is ignored, no other codec is parsed), so this is a fixed constant rather than per-camera state.
const VIDEO_CODEC_H264: &str = "h264";

/// Opus sample rate (Hz) the controller advertises in `ChangeVideoSettings` `avSerializer.parameters.opusSampleRate`. Confirmed against the Protect 7.1.77 source: `service.js` `OPUS_SAMPLE_RATE_HZ = { DEFAULT: 24e3, EDGE: 16e3 }`, and `getOpusSampleRate` returns `DEFAULT` for non-Edge cameras (the G5 Bullet is not an Edge model). Audio is ignored downstream (FLV audio tags are skipped), but the field is sent to match the real controller's payload so the camera's adoption state machine completes.
const OPUS_SAMPLE_RATE_HZ_DEFAULT: u64 = 24_000;

/// AVClient `functionName` values this module dispatches specifically. All other names fall through to the generic ok reply. The `ubnt_avclient_` prefixed forms are the camera-confirmed shapes; the bare forms are accepted defensively.
...
/// Controller→camera command that configures the camera's video stream destinations. The payload shape is reverse-engineered, not confirmed against a live camera capture. Sending it with an `extendedFlv` `avSerializer` whose `destinations` points at `tcp://<controller>:7550` is what makes the camera open the 7550 streaming channel.
const FN_CHANGE_VIDEO_SETTINGS: &str = "ChangeVideoSettings";

/// Controller→camera parameter-agreement command. The controller sends this to negotiate protocol features (status codes, heartbeats) before driving adoption forward. The camera ignores `ChangeVideoSettings` sent immediately after `timeSync` and stays in a `timeSync` liveness loop, so the adoption driver sends `paramAgreement` before `ChangeVideoSettings`.
const FN_PARAM_AGREEMENT: &str = "ubnt_avclient_paramAgreement";

/// `paramAgreement` payload field: whether the controller uses numeric status codes in replies.
const FIELD_ENABLE_STATUS_CODES: &str = "enableStatusCodes";
/// `paramAgreement` payload field: whether to use WS-level heartbeats (the camera's `ping-0` keepalive is handled separately).
const FIELD_USE_HEARTBEATS: &str = "useHeartbeats";
/// `paramAgreement` payload field: heartbeat timeout in milliseconds (confirmed against the Protect 7.1.77 source: `service.js` sends `heartbeatsTimeoutMs: 6e4` — 60000 ms — alongside `useHeartbeats: false`).
const FIELD_HEARTBEATS_TIMEOUT_MS: &str = "heartbeatsTimeoutMs";

/// `paramAgreement` `heartbeatsTimeoutMs` value the controller advertises. The real Protect controller sends `60000` (`service.js`, Protect 7.1.77); with `useHeartbeats: false` the camera does not run an AVClient-application watchdog. The real liveness mechanism is the controller's 15s WS Ping (see [`build_heartbeat_frame`]).
const HEARTBEATS_TIMEOUT_MS: u64 = 60_000;

/// The extendedFlv serializer type label (`avSerializer.type == "extendedFlv"`). Only streams with this type and a non-empty `destinations` are pushed.
...
/// Sequential adoption state machine. The real Protect controller sends adoption messages one at a time, waiting for each camera ack before sending the next. Sequential adoption respects the camera's expected request→ack→request→ack cadence.
...
enum AdoptionState {
    ...
    /// Adoption complete. The session is in steady state; only heartbeats and reactive replies are sent. `ChangeVideoSettings` is sent fire-and-forget (`responseExpected: false`), so adoption is marked complete immediately after sending it — the controller does not wait for a camera ack.
    Adopted,
}
...
pub struct AvClientSession<RW> {
    ...
    now_ms: Clock,
    ready: bool,
    /// Optional 7550 stream destination URI (`tcp://<controller_ip>:7550?retryInterval=1&connectTimeout=5`). When set, the session drives a sequential adoption sequence (`paramAgreement` → wait for ack → `ChangeVideoSettings` fire-and-forget → adopted) after `hello`. `None` ⇒ the session is purely reactive.
    stream_destination: Option<String>,
    /// Optional `streamName` for the `ChangeVideoSettings` payload, conventionally `<MAC_NO_COLONS>_<idx>`. `None` ⇒ `DEFAULT_0`.
    stream_name: Option<String>,
    /// Sequential adoption state machine.
    ...
    /// True once the camera has sent `hello` (the post-timeSync handshake advancement). The adoption driver fires after this, not after `timeSync`: sending the adoption sequence right after `timeSync` causes the camera to reset, while waiting for `hello` lets the handshake complete.
    hello_received: bool,
    ...
    /// Optional frame tracer (diagnostic). `None` in production.
...
impl<RW: Read + Write> AvClientSession<RW> {
    ...
    /// Installs a frame tracer that receives every frame read from or written to the wire. Builder-style; returns `self` for chaining. Used for diagnostic tracing of the exact AVClient exchange; production code leaves it unset (zero overhead — the `Option` is never read).
    ...
    /// Configures the 7550 stream destination so the session sends a controller-initiated `ChangeVideoSettings` (telling the camera to push extendedFlv to `stream_destination`) once the `timeSync` exchange completes. `stream_name` is the `avSerializer.parameters.streamName` (conventionally `<MAC_NO_COLONS>_<idx>`). Builder-style; returns `self` for chaining off `new`. The payload shape is reverse-engineered, not confirmed against a live camera capture.
    ...
    /// True once the session has answered a `timeSync` exchange — the signal that the camera considers the handshake complete. Stays true for the life of the session.
    pub fn is_ready(&self) -> bool {
        self.ready
    }

    /// True once the `ChangeVideoSettings` command has been sent (or skipped because no stream destination was configured). Used to confirm adoption-driving actually fired.
    ...
    /// Traces (if a tracer is installed) then encodes `frame` to the wire. Centralizes the trace-out + encode pattern so every outgoing frame is visible to a tracer without each call site repeating the trace call.
    ...
    /// Answers a WS Ping with a WS Pong control frame echoing the payload, per RFC 6455 §5.5.2. The real Protect controller (using the `ws` npm library) auto-replies to WS Pings with a WS Pong only — it does NOT send a text `pong-<N>` frame (zero occurrences of `pong-` in the Protect 7.1.77 source). The keepalive the camera sends is a WS Ping carrying the text `ping-<N>`; this session answers it with a WS Pong, not a text `pong-<N>` (which the camera would receive as a Text data frame, try to parse as AVClient JSON, fail, and tear down the 7442 session).
    ...
    fn handle_data(&mut self, payload: Vec<u8>) -> Result<(), AvClientError> {
        ...
        //
        // New requests (`timeSync`, `hello`) and unsolicited events (`EventPoorNetwork`, `EventStreamChanged`, …) are still replied to: `timeSync` carries `responseExpected: true` (it is a request that happens to chain on our previous reply's messageId, so its `inResponseTo` is nonzero but it still demands an answer), and events carry `inResponseTo: 0`. Acking events is harmless.
    ...
    /// Sends a controller-initiated `paramAgreement` command (not a reply — `inResponseTo: 0`, fresh `messageId`, `responseExpected: true`) that negotiates protocol features with the camera. The camera ignores `ChangeVideoSettings` when sent immediately after `timeSync` (it keeps looping on timeSync), so the adoption driver sends `paramAgreement` before `ChangeVideoSettings`. Payload (`enableStatusCodes: true`, `useHeartbeats: false`, `heartbeatsTimeoutMs: 60000`) is confirmed against the Protect 7.1.77 source (`service.js`: `e.send("ubnt_avclient_paramAgreement", {enableStatusCodes: true, useHeartbeats: false, heartbeatsTimeoutMs: 6e4}, ...)`).
    ...
    /// Sends a controller-initiated `ChangeVideoSettings` command (not a reply — `inResponseTo: 0`, fresh `messageId`, `responseExpected: false`, confirmed against the Protect 7.1.77 source: `service.js` `pushStream` publishes with `!1`) whose payload contains one `extendedFlv` H.264 video stream pointing at the configured 7550 destination. This is the message that makes the camera dial 7550 and push uPFLV. Payload shape is confirmed against the Protect 7.1.77 source (`service.js` `pushStream` non-UCP4 path): the video channel carries `avSerializer` plus a top-level `type: "h264"` (the codec), and `avSerializer.parameters` carries `streamName`/`withOpus`/`opusSampleRate`.
    ...
    /// Sends one controller-initiated (unsolicited) AVClient command: builds the full envelope (`from`/`to`/`functionName`/`inResponseTo:0`/fresh `messageId`/`payload`/`responseExpected`/`timestamp`) around `payload`, emits it as a Binary WS frame, and writes it to the wire. `response_expected` is `true` for `paramAgreement` (the camera acks with an `authToken`) and `false` for `ChangeVideoSettings` (fire-and-forget). Stores the messageId in `pending_adoption_msg_id` so the sequential adoption ack interceptor can match the camera's reply by `inResponseTo`.
    ...
    /// Selects the handler-specific payload for `request` and updates session state (e.g. `ready`). `timeSync` is the only camera-confirmed handler; `hello` returns the controller-identity payload (confirmed against the Protect 7.1.77 source); every other name — including `paramAgreement` / `getSystemStats` / `updateFirmwareRequest` / `stopService` / `enableLogging` — falls through to a uniform generic ok reply.
    ...
    /// The generic ok reply payload (`statusCode: 0, status: "ok", deviceID`). Uniform across all non-`timeSync`/non-`hello` handlers; the per-handler payload shapes are reverse-engineered, not confirmed against a live camera capture.
    ...
    /// The `hello` reply payload: the controller identity (`controllerName`/`controllerUuid`/`controllerVersion`), an echoed `protocolVersion` (the camera's own value from its hello payload, falling back to [`HELLO_PROTOCOL_VERSION`] if absent), and `overrideUuid: true`. Confirmed against the Protect 7.1.77 source (`service.js` `ubntAvclientHello`: `t.respond(r, {protocolVersion: g.protocolVersion, controllerName: a.name || "", controllerUuid: a.anonymousDeviceId, controllerVersion: a.version, overrideUuid: true}, false)`).
...
/// Formats `ms` (Unix milliseconds) as an ISO 8601 UTC string with millisecond precision and an explicit `+00:00` offset, matching the timestamp shape the camera sends (e.g. `2026-06-19T15:52:59.817+00:00`). The date triple comes from the shared `calendar::civil_from_days`; the time-of-day and millisecond fields are computed inline from the intra-day millisecond remainder.
...
mod tests {
    ...
    #[test]
    fn hello_protocol_version_fallback_is_67() {
    ...
    #[test]
    fn controller_message_extracts_envelope_fields() {
