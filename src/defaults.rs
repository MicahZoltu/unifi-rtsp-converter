//! Single-sourced default values shared across the proxy: the ONVIF device-information defaults (`firmware`/`serial`) and the AVClient controller-identity defaults (`controller_name`/`controller_uuid`/`controller_version`). Owned here so the dependency graph points downward — `config` (a leaf-ish module), `onvif_server`, and `protect_controller` each reference these from one place instead of `config` reaching up into two consumer modules for its `Default` impl (the prior shape, which left the leaf depending on the consumers and let two copies drift). The values themselves are unchanged; only the owning module moves.
//!
//! `onvif_server` and `protect_controller` re-export these at their own paths (`crate::onvif_server::DEFAULT_FIRMWARE`, `crate::protect_controller::DEFAULT_CONTROLLER_NAME`, …) so existing imports stay valid.
