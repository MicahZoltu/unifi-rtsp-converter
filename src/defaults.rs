//! Single-sourced default values shared across the proxy: the ONVIF device-information defaults (`firmware`/`serial`) and the AVClient controller-identity defaults (`controller_name`/`controller_uuid`/`controller_version`). Owned here so the dependency graph points downward — `config` (a leaf-ish module), `onvif_responses` (via `OnvifConfig::defaults_for`), and `protect_controller` each reference these from one place instead of `config` reaching up into two consumer modules for its `Default` impl (which would leave the leaf depending on the consumers and let two copies drift).
//!
...
/// Default controller version advertised in the AVClient `hello` reply when no override is configured. Matches the Protect package version confirmed against the Protect 7.1.77 source. Single-sourced here so `config`'s default references the same value rather than mirroring a copy that can drift.
