...
/// Hard upper bound the accept loop waits for the TLS handshake to complete. The camera sends the ClientHello promptly and each flight follows within a network RTT, so a healthy handshake finishes in well under a second. The deadline only guards against a silent peer (misrouted camera, dead socket) so a stuck accept thread does not block the listener forever; the caller logs "TLS handshake failed: timed out" and drops the connection.
const HANDSHAKE_DEADLINE_SECS: u64 = 30;

/// Sleep between `WouldBlock`/`TimedOut` retries in the handshake read loop. Keeps the spin cheap while polling well inside the deadline.
const HANDSHAKE_RETRY_SLEEP_MS: u64 = 20;

/// Per-read scratch size for the handshake read loop. The handshake flights are small (KiB-scale); this only bounds how much is buffered per stream read.
const HANDSHAKE_READ_CHUNK: usize = 8192;

/// Scratch-buffer size for the streaming `Read` path. Bounded per read; a larger encrypted record is reassembled across multiple `read` calls.
...
/// `SEC_I_CONTINUE_NEEDED` — more handshake flights needed; send the output token and continue. sspi.h.
const SEC_I_CONTINUE_NEEDED: u32 = 0x0009_0312;

/// `SEC_I_CONTEXT_EXPIRED` — peer sent `close_notify`; the stream is at EOF. sspi.h.
const SEC_I_CONTEXT_EXPIRED: u32 = 0x0009_0317;

/// `SEC_E_INCOMPLETE_MESSAGE` — the input does not contain a complete TLS record; read more bytes and retry. sspi.h.
...
/// `SCHANNEL_SHUTDOWN` control-token value passed to `ApplyControlToken` to initiate a clean `close_notify` shutdown. schannel.h.
...
/// `SECBUFFER_STREAM_TRAILER` — TLS record trailer (MAC) slot for `EncryptMessage`. sspi.h.
const SECBUFFER_STREAM_TRAILER: u32 = 6;

/// `SECBUFFER_VERSION` — `SecBufferDesc.ulVersion` value. sspi.h.
const SECBUFFER_VERSION: u32 = 0;

/// `UNISP_NAME_A` — the SSPI package name for the Microsoft Unified Security Protocol Provider (SChannel), ANSI form for `AcquireCredentialsHandleA`. schannel.h.
const UNISP_NAME_A: &[u8] = b"Microsoft Unified Security Protocol Provider\0";

// --------------------------------------------------------------------------- Raw Win32 structs (only what we call). ---------------------------------------------------------------------------

/// `TimeStamp` (sspi.h) — expiry/out parameter; we pass null where unused, but declare it for `AcquireCredentialsHandleA`. Two-DWORD form, 8 bytes, 4-byte aligned — ABI-identical to the `i64` the headers sometimes typedef it to.
#[repr(C)]
#[derive(Copy, Clone)]
struct TimeStamp {
    dwLowDateTime: u32,
...
/// `SecHandle` (sspi.h) — opaque credential/context handle pair. Both members are opaque pointers; we never dereference them.
#[repr(C)]
#[derive(Copy, Clone)]
struct SecHandle {
    dwLower: *mut c_void,
...
impl SecHandle {
    ...
    fn null() -> SecHandle {
        SecHandle { dwLower: null_mut(), dwUpper: null_mut() }
    }
}
...
impl SecBuffer {
    fn empty() -> SecBuffer {
        SecBuffer { cbBuffer: 0, BufferType: SECBUFFER_EMPTY, pvBuffer: null_mut() }
    }
}
...
/// `SCHANNEL_CRED` (schannel.h) — credential configuration. Layout matches the Windows SDK `SCHANNEL_CRED` (version 4) on x86_64: `dwVersion`, `cCreds`, `paCred` (ptr), `hRootStore` (ptr), `cMappers`, `aphMappers` (ptr), `cSupportedAlgs`, `palgSupportedAlgs` (ptr), `grbitEnabledProtocols`, minimum/maximum cipher strength, session lifespan, flags, cred format.
#[repr(C)]
...
/// `CRYPT_INTEGER_BLOB` / `CRYPT_DATA_BLOB` (wincrypt.h) — length + pointer, used for the PFX bytes and password passed to `PFXImportCertStore`.
#[repr(C)]
...
/// `SecPkgContext_StreamSizesW` (sspi.h) — returned by `QueryContextAttributesW(SECPKG_ATTR_STREAM_SIZES)`; sizes for stream-mode record framing.
#[repr(C)]
...
// --------------------------------------------------------------------------- FFI declarations. `extern "system"` matches the Win32 calling convention (stdcall on x86, the common convention on x86_64). `#[link]` pulls in the import libraries MinGW ships (`libcrypt32.a`, `libsecur32.a`). ---------------------------------------------------------------------------

#[link(name = "crypt32")]
extern "system" {
    /// `PFXImportCertStore` (wincrypt.h) — load a PFX into a transient cert store and return its handle, or NULL on failure (call `GetLastError`).
    fn PFXImportCertStore(pPFX: *mut CRYPT_INTEGER_BLOB, szPassword: *const u16, dwFlags: u32) -> *mut c_void;

    /// `CertEnumCertificatesInStore` (wincrypt.h) — enumerate certs in a store. Pass NULL for `pPrev` to get the first; pass the prior context to get the next (the prior is automatically freed). Returns NULL when done.
    fn CertEnumCertificatesInStore(hCertStore: *mut c_void, pPrev: *const CERT_CONTEXT) -> *const CERT_CONTEXT;

    /// `CertFreeCertificateContext` (wincrypt.h) — release one reference to a cert context. Returns TRUE on success.
    fn CertFreeCertificateContext(pCertContext: *const CERT_CONTEXT) -> c_int;

    /// `CertCloseStore` (wincrypt.h) — close a cert store. Outstanding cert context references (freed separately) remain valid with flag 0.
    fn CertCloseStore(hCertStore: *mut c_void, dwFlags: u32) -> c_int;
}

#[link(name = "secur32")]
extern "system" {
    /// `AcquireCredentialsHandleA` (sspi.h) — acquire an SSPI credential handle. For SChannel, `pszPackage` is `UNISP_NAME_A` and `pAuthData` points to a `SCHANNEL_CRED`.
    fn AcquireCredentialsHandleA(pszPrincipal: *const u8, pszPackage: *const u8, fCredentialUse: u32, pvLogonId: *const c_void, pAuthData: *const c_void, pGetKeyFn: *const c_void, pvGetKeyArgument: *const c_void, phCredential: *mut SecHandle, ptsExpiry: *mut TimeStamp) -> i32;

    /// `FreeCredentialsHandle` (sspi.h) — release a credential handle.
    fn FreeCredentialsHandle(phCredential: *mut SecHandle) -> i32;

    /// `AcceptSecurityContext` (sspi.h) — drive the server-side TLS handshake to completion, returning `SEC_E_OK`/`SEC_I_CONTINUE_NEEDED`/`SEC_E_INCOMPLETE_MESSAGE`/`SEC_I_CONTEXT_EXPIRED`.
    fn AcceptSecurityContext(phCredential: *const SecHandle, phContext: *const SecHandle, pInput: *mut SecBufferDesc, fContextReq: u32, TargetDataRep: u32, phNewContext: *mut SecHandle, pOutput: *mut SecBufferDesc, pfContextAttr: *mut u32, ptsExpiry: *mut TimeStamp) -> i32;

    /// `DeleteSecurityContext` (sspi.h) — release a security context.
    fn DeleteSecurityContext(phContext: *mut SecHandle) -> i32;

    /// `ApplyControlToken` (sspi.h) — apply a control token (e.g. `SCHANNEL_SHUTDOWN`) to a context before the next handshake call.
    fn ApplyControlToken(phContext: *const SecHandle, pInput: *mut SecBufferDesc) -> i32;

    /// `EncryptMessage` (sspi.h) — encrypt one plaintext into a stream-mode TLS record. The buffers' `cbBuffer` are updated on return.
    fn EncryptMessage(phContext: *const SecHandle, fQOP: u32, pMessage: *mut SecBufferDesc, MessageSeqNo: u32) -> i32;

    /// `DecryptMessage` (sspi.h) — decrypt one stream-mode TLS record. The first buffer (input) becomes consumed; a `SECBUFFER_DATA` buffer holds the plaintext and a `SECBUFFER_EXTRA` buffer holds leftover bytes.
    fn DecryptMessage(phContext: *const SecHandle, pMessage: *mut SecBufferDesc, MessageSeqNo: u32, pfQOP: *mut u32) -> i32;

    /// `FreeContextBuffer` (sspi.h) — release a buffer SSPI allocated (e.g. an output handshake token produced with `ASC_REQ_ALLOCATE_MEMORY`).
    fn FreeContextBuffer(pvContextBuffer: *mut c_void) -> i32;

    /// `QueryContextAttributesW` (sspi.h) — query a context attribute. Used here for `SECPKG_ATTR_STREAM_SIZES` to learn record framing sizes.
...
pub enum HandshakeError {
    /// The peer completed the TCP handshake but closed the connection before sending any application bytes — i.e. `read()` returned 0 on the very first read of the handshake, with no prior bytes received. Common for embedded clients: the UVC G5 camera performs this exact zero-byte TCP liveness probe on port 7442 before opening the real TLS connection. No TLS handshake was attempted (SChannel never received a `ClientHello`). Callers should treat this as benign — debug log, no error metric.
    PeerClosedBeforeData,
    /// The TLS handshake failed for a real reason: SChannel rejected the peer's bytes, the peer closed mid-handshake (after sending some bytes), the handshake timed out, or a write to the peer failed. Callers should log this at error level and count it.
    Failed(io::Error),
}
...
impl fmt::Display for HandshakeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            HandshakeError::PeerClosedBeforeData => f.write_str("peer closed before sending any bytes (TCP liveness probe)"),
...
// --------------------------------------------------------------------------- TlsAcceptor — built once from a PFX; Clone (Arc-wrapped cred handle) so listener threads share it. ---------------------------------------------------------------------------

/// Server-side SChannel credential built from a PFX. `Clone` is cheap (one `Arc` refcount) so a listener hands the same acceptor to every accepted connection. Dropping the last clone frees the credential handle and the loaded certificate context.
#[derive(Clone)]
...
/// SChannel credential handles are safe to share across threads for inbound accepts (the package serializes internally; the handle is read-only during `AcceptSecurityContext`). We hold the certificate context for the cred's lifetime and never mutate either from Rust.
unsafe impl Send for RawAcceptor {}
unsafe impl Sync for RawAcceptor {}

impl Drop for RawAcceptor {
    fn drop(&mut self) {
        // Order: free the credential (which may still reference the cert) before freeing the certificate context itself.
        unsafe {
...
impl TlsAcceptor {
    ...
    pub fn from_pfx(pfx: &[u8], password: Option<&str>) -> io::Result<TlsAcceptor> {
        unsafe {
            ...
            let pw_ptr = if pw_wide.is_empty() { null::<u16>() } else { pw_wide.as_ptr() };
            // flags = 0: persist the key to the user's CSP/KSP container. See the method doc for why PKCS12_NO_PERSIST_KEY is not used.
            ...
            if cert.is_null() {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "PFX contains no certificates"));
            }
            ...
            let ret = AcquireCredentialsHandleA(null(), UNISP_NAME_A.as_ptr(), SECPKG_CRED_INBOUND, null(), &cred_data as *const SCHANNEL_CRED as *const c_void, null(), null(), &mut cred_handle, null_mut());
            if ret as u32 != SEC_E_OK {
                let _ = CertFreeCertificateContext(cert);
                return Err(io::Error::other(format!("AcquireCredentialsHandleA failed: 0x{:08X}", ret as u32)));
            }

            Ok(TlsAcceptor { inner: Arc::new(RawAcceptor { cred: cred_handle, cert }) })
        }
    }
    ...
    pub fn accept<S: Read + Write>(&self, stream: S) -> Result<TlsStream<S>, HandshakeError> {
        ...
        loop {
            // Read before the first call so SChannel is never handed an empty token (which would also throw off the first_call/INCOMPLETE_MESSAGE invariant). Subsequent iterations only read when the prior result was CONTINUE_NEEDED with no leftover, or INCOMPLETE_MESSAGE.
            if input.is_empty() {
            ...
            // phContext is NULL on the very first call, then the live context handle on every subsequent *valid* call. phNewContext always aliases the same `context` slot (SChannel updates it in place). Using raw pointers avoids overlapping a shared and mutable borrow of `context` in the same FFI call.
            let ctx_ptr: *mut SecHandle = &mut context;
            let ph_context: *const SecHandle = if first_call { null::<SecHandle>() } else { ctx_ptr };
            let mut attrs: u32 = 0;
            ...
            for (i, buf) in out_bufs.iter_mut().enumerate() {
                if !buf.pvBuffer.is_null() && buf.cbBuffer > 0 {
                    ...
                    if i == 0 {
                        // Only the TOKEN buffer is a handshake flight the peer expects to receive; an ALERT is SChannel-internal and already folded into the TOKEN flight when sent.
            ...
            match ret_u {
                SEC_E_OK => {
                    ...
                    return TlsStream::new(stream, context, leftover, self.inner.clone()).map_err(HandshakeError::Failed);
                }
                ...
                SEC_E_INCOMPLETE_MESSAGE => {
                    // Do NOT flip first_call — the context is not yet live and the next call must still pass phContext = NULL. Just read more bytes and retry.
                    if let Err(e) = read_more(&mut stream, &mut input, deadline) {
                        return Err(read_err_to_handshake(e, received_any));
                    }
                    received_any = true;
                ...
                SEC_I_CONTEXT_EXPIRED => {
                    return Err(HandshakeError::Failed(io::Error::new(io::ErrorKind::ConnectionAborted, "peer closed during TLS handshake")));
                }
                other => {
                    return Err(HandshakeError::Failed(io::Error::other(format!("AcceptSecurityContext failed: 0x{other:08X}"))));
                }
            }
        }
    }
...
/// Maps a `read_more` error to a [`HandshakeError`]: a peer EOF (`Ok(0)` from `read`) when *no* bytes have ever been received on this connection is a benign zero-byte TCP liveness probe (`PeerClosedBeforeData`); any other error (EOF after some bytes, timeout, fatal read error) is a real failure.
...
/// Returns the `cbBuffer` of the first `SECBUFFER_EXTRA` entry in `bufs`, or 0 if none. The EXTRA buffer holds input bytes SSPI did not consume (leftover for the next call).
...
/// Reads more bytes from `stream` into `input`, retrying on `Interrupted` and tolerating `WouldBlock`/`TimedOut` (sleep + retry) until `deadline` elapses. Returns `Ok(())` after appending at least one byte, or an error on EOF / timeout / fatal read error.
...
/// A TLS stream wrapping an underlying `Read + Write` byte stream. Encrypts outgoing writes into TLS records via `EncryptMessage` and decrypts incoming reads via `DecryptMessage`, carrying leftover encrypted/decrypted bytes across calls so records spanning reads and multiple records per read are handled correctly.
pub struct TlsStream<S> {
    stream: S,
    context: SecHandle,
    /// The credential handle (Arc-shared with `TlsAcceptor`) — retained so the `shutdown()` path can pass it to `AcceptSecurityContext`, which SChannel requires even on the post-handshake shutdown call (passing NULL yields `SEC_E_INVALID_HANDLE` / 0x80090301). Matches the `schannel` crate's `TlsStream` which holds `cred: SchannelCred` for the same reason.
...
impl<S: Read + Write> TlsStream<S> {
    /// Finishes a completed handshake: stores the context, queries the stream-mode record sizes, and seeds `enc_buf` with any bytes the peer sent immediately after the final handshake flight (post-handshake app data that arrived in the same TCP segment).
    fn new(stream: S, context: SecHandle, leftover_enc: Vec<u8>, cred: Arc<RawAcceptor>) -> io::Result<TlsStream<S>> {
        let sizes = query_stream_sizes(&context)?;
        Ok(TlsStream { stream, context, cred, sizes, dec_buf: Vec::new(), dec_pos: 0, enc_buf: leftover_enc, shutdown_done: false })
    }
    ...
    /// Drives a clean TLS `close_notify` shutdown: applies the `SCHANNEL_SHUTDOWN` control token, calls `AcceptSecurityContext` once more to produce the close_notify flight, sends it, and is then safe to drop. Idempotent — subsequent calls are no-ops.
    pub fn shutdown(&mut self) -> io::Result<()> {
        ...
        unsafe {
            ...
            let mut shut_buf = SecBuffer { cbBuffer: std::mem::size_of::<u32>() as u32, BufferType: SECBUFFER_TOKEN, pvBuffer: token_ptr };
            let mut shut_desc = SecBufferDesc { ulVersion: SECBUFFER_VERSION, cBuffers: 1, pBuffers: &mut shut_buf };
            ...
            let ret = AcceptSecurityContext(&self.cred.cred as *const SecHandle, ctx_ptr, &mut in_desc, ASC_REQ_FLAGS, 0, ctx_ptr, &mut out_desc, &mut attrs, null_mut());
            if ret as u32 != SEC_E_OK && ret as u32 != SEC_I_CONTEXT_EXPIRED {
                return Err(io::Error::other(format!("AcceptSecurityContext(shutdown) failed: 0x{:08X}", ret as u32)));
            }
            // Send the close_notify flight and free any SSPI-allocated output buffers (TOKEN at [0], ALERT at [1], fill-in at [2]).
            for (i, buf) in out_bufs.iter_mut().enumerate() {
                if !buf.pvBuffer.is_null() && buf.cbBuffer > 0 {
                    if i == 0 {
                        let token = std::slice::from_raw_parts(buf.pvBuffer as *const u8, buf.cbBuffer as usize);
    ...
    /// Encrypts one plaintext chunk (`<= cbMaximumMessage`) into a TLS record and writes the whole record (header + encrypted data + trailer) to the underlying stream. The record is fully sent before returning, so there is no half-sent-record state to recover from.
    fn encrypt_and_send(&mut self, plaintext: &[u8]) -> io::Result<()> {
        ...
        // SAFETY: `base..base+total` is the valid allocation backing `record`; `header` and `header+plaintext.len()` lie within it, so the offset pointers are in-bounds. The pointers alias `record` only until `EncryptMessage` returns; we do not reallocate or alias-uniquely during the call.
        let (data_ptr, trailer_ptr) = unsafe { (base.add(header), base.add(header + plaintext.len())) };
        let mut bufs = [SecBuffer { cbBuffer: header as u32, BufferType: SECBUFFER_STREAM_HEADER, pvBuffer: base as *mut c_void }, SecBuffer { cbBuffer: plaintext.len() as u32, BufferType: SECBUFFER_DATA, pvBuffer: data_ptr as *mut c_void }, SecBuffer { cbBuffer: trailer as u32, BufferType: SECBUFFER_STREAM_TRAILER, pvBuffer: trailer_ptr as *mut c_void }, SecBuffer::empty()];
        let mut desc = SecBufferDesc { ulVersion: SECBUFFER_VERSION, cBuffers: bufs.len() as u32, pBuffers: bufs.as_mut_ptr() };
        let ret = unsafe { EncryptMessage(&self.context, 0, &mut desc, 0) };
        if ret as u32 != SEC_E_OK {
            return Err(io::Error::other(format!("EncryptMessage failed: 0x{:08X}", ret as u32)));
        }
        // After encryption the header/data/trailer cbBuffer reflect the actual record framing (data may have grown with padding folded in). Send the contiguous record = header + data + trailer.
    ...
    /// Attempts one `DecryptMessage` over `enc_buf`. On success copies the decrypted plaintext into `dec_buf`, retains any leftover encrypted bytes in `enc_buf`, resets the decrypt read position, and returns `Ok(true)`. Returns `Ok(false)` when more encrypted bytes are needed (`SEC_E_INCOMPLETE_MESSAGE`) so the caller reads more. Returns `Ok(Err(eof))` semantics via the special `CONTEXT_EXPIRED` variant.
    fn try_decrypt(&mut self) -> io::Result<DecryptOutcome> {
        let pos = self.enc_buf.len();
        let mut bufs = [SecBuffer { cbBuffer: pos as u32, BufferType: SECBUFFER_DATA, pvBuffer: self.enc_buf.as_mut_ptr() as *mut c_void }, SecBuffer::empty(), SecBuffer::empty(), SecBuffer::empty()];
        let mut desc = SecBufferDesc { ulVersion: SECBUFFER_VERSION, cBuffers: bufs.len() as u32, pBuffers: bufs.as_mut_ptr() };
        let mut qop: u32 = 0;
        let ret = unsafe { DecryptMessage(&self.context, &mut desc, 0, &mut qop) };
        match ret as u32 {
            SEC_E_OK => {
                // Copy decrypted plaintext out before we touch enc_buf (the DATA pointer aliases enc_buf's storage).
                self.dec_buf.clear();
                if !bufs[1].pvBuffer.is_null() && bufs[1].cbBuffer > 0 {
                    let data = unsafe { std::slice::from_raw_parts(bufs[1].pvBuffer as *const u8, bufs[1].cbBuffer as usize) };
                    self.dec_buf.extend_from_slice(data);
                }
                self.dec_pos = 0;
                let extra = if bufs[3].BufferType == SECBUFFER_EXTRA { bufs[3].cbBuffer as usize } else { 0 };
            ...
            other => Err(io::Error::other(format!("DecryptMessage failed: 0x{other:08X}"))),
        }
    }
}
...
impl<S> Drop for TlsStream<S> {
    fn drop(&mut self) {
        // Best-effort: do not attempt the close_notify exchange from Drop (the peer may already be gone); just release the SSPI context.
        unsafe {
            ...
            if !self.enc_buf.is_empty() {
                match self.try_decrypt()? {
                    DecryptOutcome::Decrypted => {
                        ...
                        // Decryption produced no plaintext (e.g. a control record); loop to decrypt the next record or read more.
                        continue;
                    }
...
/// Queries `SECPKG_ATTR_STREAM_SIZES` for the context, returning the record framing sizes used by `encrypt_and_send`.
fn query_stream_sizes(context: &SecHandle) -> io::Result<SecPkgContext_StreamSizesW> {
    let mut sizes: SecPkgContext_StreamSizesW = SecPkgContext_StreamSizesW { cbHeader: 0, cbTrailer: 0, cbMaximumMessage: 0, cBuffers: 0, cbBlockSize: 0 };
    let ret = unsafe { QueryContextAttributesW(context as *const SecHandle, SECPKG_ATTR_STREAM_SIZES, &mut sizes as *mut SecPkgContext_StreamSizesW as *mut c_void) };
    if ret as u32 == SEC_E_OK {
        Ok(sizes)
    } else {
        Err(io::Error::other(format!("QueryContextAttributesW(STREAM_SIZES) failed: 0x{:08X}", ret as u32)))
    }
}
