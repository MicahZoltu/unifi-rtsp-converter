//! Camera listener and FLV pipeline. Binds the camera push port (7550 in production), accepts a single active connection at a time (force-closing the prior one when a new one arrives), optionally strips the uPFLV prefix (absent on 7550 — the production transport is bare FLV), parses the FLV header, frames the tag stream, and dispatches video/script tags into the shared `StreamState`.
//!
//! The real 7550 transport is **plain TCP, bare FLV** — no TLS, no WebSocket, no uPFLV prefix. The camera sends `FLV\x01\x07\x00\x00\x00\x09` (the standard FLV header) directly over a raw TCP socket. `run_connection` drives this: it sets the per-connection socket options (nodelay + bounded read timeout so the shutdown flag is polled promptly), calls `detect_and_strip_prefix` (a no-op when the stream starts with `FLV` instead of the uPFLV prefix), and feeds the bare FLV bytes directly to `FlvParser`.
//!
        ...
        if let Some(ref mut p) = parser {
            match p.push(chunk) {
                Ok(events) => {
                    ...
                    // After an empty `Ok` the framer may be parked in `Resyncing` awaiting a `resync()` attempt (the last push returned `Err` and resync found no boundary yet; more bytes have now arrived). Drive it here so recovery is prompt rather than waiting for the next error.
                    if p.is_resyncing() {
