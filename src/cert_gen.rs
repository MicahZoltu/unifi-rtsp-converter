//! Self-signed PFX auto-generation (plan step 05). Generates a fresh self-signed RSA cert + key via raw CryptoAPI FFI and exports it as a no-password PKCS#12 (PFX) so an operator never has to run `openssl` by hand before the proxy starts. Generation fires in two places: proactively during `--install` (`service::win::install`) so the first `sc start` always finds a cert, and lazily during `App::bootstrap` if the configured PFX is absent and the exe directory is writable (a `--console` convenience). Each generation produces a unique random keypair; the camera does not validate the server cert (SChannel is configured for no chain validation in `tls_schannel`), so rotation is free and no CA coordination is needed.
//!
...
//!
//! Implementation note: each generation creates a uniquely-named CryptoAPI key container in the user's CSP profile (transient RSA key material). The container is not deleted after export (CryptoAPI's `CRYPT_DELETEKEYSET` delete is best-effort and awkward to interleave with the still-open provider handle); because the container name embeds a nanosecond timestamp it never collides, and generation is a rare event (once per `--install`, or once on the first `--console` after the PFX is removed), so the accumulation is negligible. The exported PFX is self-contained and does not depend on the container after export.
