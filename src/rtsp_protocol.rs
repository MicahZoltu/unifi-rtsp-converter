...
pub(crate) const SESSION_TIMEOUT_SECS: u32 = 60;
...
/// RTSP status code `200 OK`, per RFC 2326 §A.1. `pub(crate)` so the runtime's response-building and status-checking call sites share the protocol's single source.
pub(crate) const STATUS_OK: u16 = 200;
/// RTSP status code `400 Bad Request`, per RFC 2326 §17.4.1. `pub(crate)` for the runtime's malformed-request close path.
pub(crate) const STATUS_BAD_REQUEST: u16 = 400;
/// RTSP status code `454 Session Not Found`, per RFC 2326 §17.3.7.
pub(crate) const STATUS_SESSION_NOT_FOUND: u16 = 454;
/// RTSP status code `461 Unsupported transport`, per RFC 2326 §17.4.16. `pub(crate)` for the runtime's per-request warn-log filter.
pub(crate) const STATUS_UNSUPPORTED_TRANSPORT: u16 = 461;
/// RTSP status code `501 Not Implemented`, per RFC 2326 §17.4.5.
pub(crate) const STATUS_NOT_IMPLEMENTED: u16 = 501;
/// RTSP status code `503 Service Unavailable`, per RFC 2326 §17.4.17. `pub(crate)` for the runtime's flood-rejection and DESCRIBE-before-camera paths.
pub(crate) const STATUS_SERVICE_UNAVAILABLE: u16 = 503;
...
/// Extracts the session id (the token before any `;` parameter) from a `Session:` header value, per RFC 2326 §12.37. `pub(crate)` so the runtime's SETUP wiring can recover the id from the `Session:` header value it built.
pub(crate) fn session_id(value: &str) -> String {
...
/// Builds a response with the canonical reason phrase for `code`. `pub(crate)` so the runtime can build the `503` flood-rejection and `400` malformed-request close responses without re-implementing the reason-phrase lookup.
pub(crate) fn response(code: u16, cseq: Option<u32>, session: Option<String>, headers: Vec<(String, String)>, body: Vec<u8>) -> RtspResponse {
...
impl RtspSessions {
    ...
    /// True iff any session on this connection has been advanced to `playing`. The runtime's idle-timeout reaper uses this to exempt an actively-streaming connection from control-channel-silence reaping. `pub(crate)` for that one cross-module caller.
