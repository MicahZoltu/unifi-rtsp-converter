//! Integration tests for `flvproxy::sdp` (step 09): the hand-rolled Base64
//! encoder, `profile-level-id` derivation, and the full `build_sdp` body.
//! Covers the exact cases enumerated in `plan/09-sdp-generation.md`,
//! asserting byte-for-byte / string-for-string output. Expected SDP bodies
//! are built from independent string literals — only `sprop-parameter-sets`
//! and `profile-level-id` are computed via the module's own public helpers,
//! so the tests stay self-consistent without hard-coding magic.
...
/// `4D 40 1F` (Main profile, level 3.1), matching the plan's example.
...
/// Builds `CodecParams` carrying `SPS`/`PPS` and the given advertised frame
/// rate. Width/height are left unknown since the SDP does not encode them.
...
/// Asserts every line of `sdp` is `\r\n`-terminated and the body ends with a
...
fn build_sdp_without_fps_omits_framerate_line() {
    ...
    assert_eq!(sdp, expected);
    assert!(
        !sdp.contains("a=framerate"),
        "no framerate line when fps is None"
    );
    assert_crlf_well_formed(&sdp);
}
...
fn build_sdp_falls_back_to_baseline_profile_level_id_for_short_sps() {
    ...
    assert!(
        ...
        "short SPS must advertise the fallback profile-level-id: {sdp}",
    );
