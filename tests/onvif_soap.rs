//! Integration tests for `flvproxy::onvif_server` (step 22): the SOAP router
//! (`route`) at the string level and the HTTP runtime (`OnvifServer`) over
//! loopback TCP. Covers the cases enumerated in `plan/22-onvif-soap.md`:
//! `GetCapabilities`, `GetDeviceInformation`, `GetProfiles`, `GetStreamUri`,
//! the unknown-action SOAP Fault, XML-escaping of the server IP, the
//! `SOAPAction`-header vs body-namespace routing fallback, and one
...
/// Realistic-ish SPS with NALU header `0x67` and profile/compat/level
/// `4D 40 1F` (Main profile, level 3.1), matching the RTSP/SDP tests.
...
/// Builds `CodecParams` carrying `SPS`/`PPS`, 1280x720, 25 fps, so the
/// `GetProfiles` test asserts the metadata-derived values rather than the
/// 1920x1080@30 fallback.
...
/// Envelope wrapping an empty body for the given SOAP action, used so the
/// router's body-namespace fallback has something to scan.
...
fn get_capabilities_response_advertises_device_and_media_xaddrs() {
    ...
    assert!(
        xml.contains(
            "<tds:Device><tt:XAddrs>http://127.0.0.1:8080/onvif/device_service</tt:XAddrs>"
        ),
        "device XAddrs must point at the device service: {xml}"
    );
    assert!(
        xml.contains("/onvif/media_service</tt:XAddrs>"),
        "media XAddrs must point at the media service: {xml}"
    );
}
...
fn unknown_soap_action_yields_action_not_supported_fault() {
    ...
    assert!(
        xml.contains("ActionNotSupported"),
        "unknown action must return a SOAP Fault: {xml}"
    );
}
...
fn missing_soap_action_routes_via_body_namespace_fallback() {
    ...
    assert!(
        xml.contains("<tt:Uri>rtsp://127.0.0.1:8554/stream</tt:Uri>"),
        "body-namespace fallback must still route GetStreamUri: {xml}"
    );
}
...
fn server_ip_with_markup_is_xml_escaped_in_responses() {
    ...
    assert!(
        !caps.contains(injected),
        "raw injected IP must not appear unescaped: {caps}"
    );
    assert!(
        caps.contains("10.0.0.1&amp;&lt;&gt;&quot;&apos;"),
        "injected IP must be escaped: {caps}"
    );
    let (_status, uri) = route(
        "\"http://www.onvif.org/ver10/media/wsdl/GetStreamUri\"",
        "",
        &cfg,
        &StreamState::new(),
    ...
    assert!(
        !uri.contains(injected),
        "raw injected IP must not appear in stream URI: {uri}"
    );
    assert!(
        uri.contains("rtsp://10.0.0.1&amp;&lt;&gt;&quot;&apos;:8554/stream"),
        "stream URI must escape injected IP: {uri}"
    );
}
...
/// Spins up an `OnvifServer` on an ephemeral loopback listener, returning the
/// bound address and a shutdown flag. The server thread is detached; dropping
/// the guard stops the accept loop.
struct Harness {
...
/// Sends one SOAP request over a fresh TCP connection and returns the full
/// HTTP response (status line + headers + body). `soap_action` is written
/// verbatim into the `SOAPAction:` header; pass an empty string to omit it.
...
/// Returns true once the response buffer holds a full header block AND its
...
fn http_get_stream_uri_with_soap_action_header_returns_200_and_rtsp_uri() {
    ...
    assert!(
        resp.contains("Content-Type: application/soap+xml"),
        "content type must be SOAP: {resp}"
    );
    assert!(
        resp.contains("<tt:Uri>rtsp://127.0.0.1:8554/stream</tt:Uri>"),
        "body must contain the RTSP URI: {resp}"
    );
}
...
fn http_get_stream_uri_without_soap_action_routes_via_body_namespace() {
    ...
    assert!(
        resp.contains("<tt:Uri>rtsp://127.0.0.1:8554/stream</tt:Uri>"),
        "body-namespace fallback must still return the URI: {resp}"
    );
}
