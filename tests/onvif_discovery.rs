...
use flvproxy::onvif_discovery::{build_bye, build_hello, build_probe_match, parse_probe, random_device_addr, Discovery, DiscoveryConfig};
...
/// Upper bound for "within a short timeout" assertions against the loopback multicast round-trip. Discovery is polled every 50 ms, so 2 s is generous.
...
/// Builds a synthetic WS-Discovery `Probe` SOAP envelope carrying the given `wsa:MessageID`, matching the shape an NVR sends.
...
fn build_probe_match_contains_xaddr_endpoint_and_network_video_transmitter() {
    ...
    assert!(xml.contains("tns:NetworkVideoTransmitter"), "NetworkVideoTransmitter type missing: {xml}");
    ...
    assert!(xml.contains("onvif://www.onvif.org/Profile/Streaming"), "Streaming scope missing: {xml}");
    assert!(xml.contains("http://schemas.xmlsoap.org/ws/2005/04/discovery/ProbeMatches"), "ProbeMatch wsa:Action missing: {xml}");
}
...
fn build_hello_contains_hello_action_and_xaddr() {
    ...
    assert!(xml.contains("http://schemas.xmlsoap.org/ws/2005/04/discovery/Hello"), "Hello action missing: {xml}");
    assert!(xml.contains("<wsdiscovery:Hello>"), "Hello element missing: {xml}");
    assert!(xml.contains(XADDR), "XAddr missing from Hello: {xml}");
    assert!(xml.contains("tns:NetworkVideoTransmitter"), "NetworkVideoTransmitter missing: {xml}");
}

#[test]
fn build_bye_contains_bye_action_and_xaddr() {
    let xml = build_bye(XADDR, DEVICE_ADDR);
    assert!(xml.contains("http://schemas.xmlsoap.org/ws/2005/04/discovery/Bye"), "Bye action missing: {xml}");
...
fn discovery_replies_to_probe_with_probe_match_over_loopback_multicast() {
    let probe_socket = match join_multicast_for_probe() {
        Ok(s) => s,
        Err(_) => {
            eprintln!("skip: this environment does not support loopback multicast join; XML-builder tests above cover the protocol surface");
            return;
        }
    };
    ...
    probe_socket.send_to(&probe_envelope("urn:uuid:probe-loopback"), &dst).expect("send probe");
    ...
    let reply = got.expect("ProbeMatch reply not received within deadline; if this environment lacks loopback multicast, the test should have skipped at the join step");
    assert!(reply.contains(XADDR), "reply must advertise the XAddr: {reply}");
    assert!(reply.contains("tns:NetworkVideoTransmitter"), "reply must advertise NetworkVideoTransmitter: {reply}");
    assert!(reply.contains("urn:uuid:probe-loopback"), "reply must echo the probe MessageID via RelatesTo: {reply}");
    assert!(reply.contains("10.0.0.5"), "reply must carry the configured server IP: {reply}");
    // Silence unused warning on probe_local when the assertion path doesn't reference it explicitly — it documents the source address the reply is expected to land at.
    let _ = probe_local;
}

/// Sends a `Bye`-shaped datagram (not a Probe) to the multicast group and asserts the `Discovery` does not reply within a short window — a Probe-only responder must stay silent on non-Probe traffic.
#[test]
fn discovery_does_not_reply_to_non_probe_datagram() {
    ...
    probe_socket.set_read_timeout(Some(Duration::from_millis(500))).expect("set read timeout");
    probe_socket.send_to(bye.as_bytes(), format!("{MULTICAST_GROUP}:{MULTICAST_PORT}")).expect("send bye");
    ...
    assert!(!received_probe_match, "Discovery must not reply to a non-Probe datagram");
}

/// Spawns a `DiscoveryWithLogger` and asserts it logs the joined-group INFO line and the stopped INFO line on shutdown, confirming the logger path wires end to end. Uses a temp log file. Skips (does not fail) when the WS-Discovery multicast port is already bound by a parallel test or when loopback multicast is unavailable — the join-line logging is verified by the runtime path that does obtain the port.
#[test]
fn discovery_with_logger_emits_join_and_stop_lines() {
    ...
    // The runtime binds 0.0.0.0:3702; if a parallel test holds it, the bind fails and the runtime logs a WARN rather than the join INFO. Detect that and skip, since the logger wiring itself is what we are testing.
    let probe_socket = match join_multicast_for_probe() {
        Ok(s) => s,
        Err(_) => {
    ...
    let log_path = std::env::temp_dir().join(format!("flvproxy-wsdiscovery-{}.log", std::process::id()));
    ...
    if !log_text.contains("wsdiscovery: joined 239.255.255.250:3702") {
        eprintln!("skip: runtime could not bind the multicast port (likely parallel test); log was: {log_text}");
        let _ = std::fs::remove_file(&log_path);
        return;
    }
    assert!(log_text.contains("wsdiscovery: stopped"), "log must record the stop line: {log_text}");
    let _ = std::fs::remove_file(&log_path);
}

/// Binds a `UdpSocket` on an ephemeral port, joins the WS-Discovery multicast group, and enables multicast loopback so the probe socket receives its own multicast sends and the Discovery's unicast replies. Returns `Err` when the environment does not support loopback multicast (CI without a multicast-capable loopback interface), so the caller can skip rather than fail.
fn join_multicast_for_probe() -> Result<UdpSocket, std::io::Error> {
    let socket = UdpSocket::bind("0.0.0.0:0")?;
    let group: std::net::Ipv4Addr = MULTICAST_GROUP.parse().map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidInput, "bad group"))?;
