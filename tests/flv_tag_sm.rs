//! Integration tests for `flvproxy::flv_parser` step 03: the push-based FLV
//! tag framing state machine. Covers the exact cases enumerated in
//! `plan/03-flv-tag-state-machine.md`, asserting byte-for-byte event bodies
//! and exact timestamp values.
...
/// Video-tag (`0x09`) payload: a minimal standard AVC keyframe header
...
fn timestamp_extended_field_combines_into_full_u32_with_rollover() {
    // First tag: low 24 bits all set, ext 0 → 0x00FFFFFF.
    // Second tag: low 0, ext 1 → 0x01000000 (the rollover point).
    let mut s = vec![0, 0, 0, 0];
...
fn oversized_data_size_returns_error_without_allocating_body() {
    // 12 MiB declared payload — above the 8 MiB cap, and a valid u24
    // (0xC00000 ≤ 0xFFFFFF) since the FLV `data_size` field is only 3 bytes.
    // No body bytes are appended, so the only way this could allocate the
    // body is if the framer ignored the cap; the error must fire first.
...
fn empty_payload_emits_event_with_empty_body_and_consumes_following_prev_size() {
    // A zero-length script tag followed by a normal video tag. Producing the
    // second event proves the previous-tag-size after the empty tag was read
    // and the framer advanced correctly.
    let mut s = vec![0, 0, 0, 0];
