//! Integration tests for `flvproxy::rtsp_server` (step 10): RTSP request
//! parsing, response serialization, the five method handlers, and transport
//! negotiation. Covers every case enumerated in `plan/10-rtsp-protocol.md`,
//! asserting exact status codes, header values, session-state transitions,
//! and byte-for-byte response wire format.

use flvproxy::rtsp_server::{
    handle_describe, handle_options, handle_play, handle_setup, handle_teardown, parse_request,
    Method, RtspRequest, RtspResponse, RtspSessions, Transport,
};
...
/// `4D 40 1F` (Main profile, level 3.1), matching the SDP tests.
...
/// Parses a complete request from a UTF-8 string, asserting the buffer was
/// fully consumed and no error occurred.
fn parse_full(text: &str) -> RtspRequest {
    let (req, consumed) = parse_request(text.as_bytes())
...
/// Returns the value of the first header named `name` (case-insensitive) in
/// `resp.headers`, or `None`. Note the `Session` header lives on
/// `resp.session`; use `session_header` for it.
...
/// Extracts the bare session id (text before any `;`) from a `Session:`
/// header value.
...
fn parse_describe_captures_accept_header() {
    let req = parse_full(
        "DESCRIBE rtsp://server:8554/stream RTSP/1.0\r\nCSeq: 1\r\nAccept: application/sdp\r\n\r\n",
    );
...
fn parse_setup_captures_transport_header_case_insensitively() {
    let req = parse_full(
        "SETUP rtsp://server:8554/stream/streamid=0 RTSP/1.0\r\nCSeq: 2\r\nTransport: RTP/AVP/TCP;unicast;interleaved=0-1\r\n\r\n",
    );
...
fn handle_setup_tcp_interleaved_echoes_transport_and_stores_interleaved_session() {
    ...
    assert!(
        session_header.contains(";timeout=60"),
        "Session header must advertise the timeout: {session_header}",
    );
    let transport = header_value(&resp, "Transport").expect("Transport header present");
    assert!(
        transport.contains("interleaved=0-1"),
        "echoed Transport must contain interleaved=0-1: {transport}",
    );
...
fn handle_setup_udp_echoes_client_and_server_ports() {
    ...
    let transport = header_value(&resp, "Transport").expect("Transport header present");
    assert!(
        transport.contains("client_port=4588-4589"),
        "echoed Transport must contain client_port=4588-4589: {transport}",
    );
    assert!(
        transport.contains("server_port="),
        "echoed Transport must contain a server_port pair: {transport}",
    );

    let session_header = session_header(&resp).expect("Session header present");
    let stored = sessions
        .get(session_id(session_header))
...
fn handle_setup_bogus_transport_returns_461() {
    ...
    assert!(
        sessions.get("anything").is_none(),
        "no session should be allocated"
    );
}

#[test]
fn handle_play_on_known_session_returns_200_with_range_and_marks_playing() {
    let setup_req = parse_full(
        "SETUP rtsp://server:8554/stream/streamid=0 RTSP/1.0\r\nCSeq: 1\r\nTransport: RTP/AVP/TCP;unicast;interleaved=0-1\r\n\r\n",
    );
    let mut sessions = RtspSessions::new();
    ...
    let play_req = parse_full(&format!(
        "PLAY rtsp://server:8554/stream RTSP/1.0\r\nCSeq: 2\r\nSession: {id}\r\n\r\n"
    ));
    ...
    assert_eq!(
        session_id(session_header(&resp).expect("Session header")),
        id
    );
...
fn handle_teardown_then_play_returns_200_then_454() {
    ...
    let teardown_req = parse_full(&format!(
        "TEARDOWN rtsp://server:8554/stream RTSP/1.0\r\nCSeq: 2\r\nSession: {id}\r\n\r\n"
    ));
    ...
    assert!(
        ...
        "session must be removed by TEARDOWN"
    );
...
fn response_to_bytes_serializes_canonical_wire_format_without_body() {
    let resp = RtspResponse {
        ...
        cseq: Some(5),
        session: Some("DEADBEEF;timeout=60".to_string()),
        headers: vec![("Public".to_string(), "OPTIONS, DESCRIBE".to_string())],
        body: Vec::new(),
    };
...
fn response_to_bytes_appends_content_length_and_body_when_body_present() {
    let resp = RtspResponse {
        ...
        cseq: Some(1),
        session: None,
        headers: vec![("Content-Type".to_string(), "application/sdp".to_string())],
        body: b"v=0\r\n".to_vec(),
    };
    let expected = b"RTSP/1.0 200 OK\r\n\
                     CSeq: 1\r\n\
...
fn response_to_bytes_omits_content_length_when_body_empty() {
    let resp = RtspResponse {
        status: 461,
        status_text: "Unsupported transport".to_string(),
        cseq: Some(9),
        session: None,
        headers: Vec::new(),
        body: Vec::new(),
...
fn parse_request_header_names_match_case_insensitively() {
    let req =
        parse_full("DESCRIBE rtsp://x RTSP/1.0\r\ncseq: 42\r\nACCEPT: application/sdp\r\n\r\n");
