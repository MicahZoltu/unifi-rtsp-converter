//! Integration tests for `flvproxy::rtsp_server` runtime half (step 11):
//! the `TcpListener` accept loop, per-session state, interleaved + UDP RTP
//! transports, the `PacketSink` test seam, and client-cleanup semantics.
//! Covers the cases enumerated in `plan/11-rtsp-server.md`.
//!
//! The server's `StreamState` is fed by the test thread itself (a mock frame
//! producer), so no real camera is involved. Loopback TCP/UDP sockets keep
//! the tests fast, deterministic, and CI-friendly.
...
/// Per-poll wait when the test needs the server to catch up to an action
/// (publish a frame, drain a dropped client). Long enough for a loopback
/// round-trip and a pump poll cycle, short enough to fail fast.
...
/// `4D 40 1F` (Main profile, level 3.1), matching the SDP/RTSP-protocol
/// tests.
...
/// Spins up an `RtspServer` on an ephemeral loopback listener, returning the
/// first client connection, the server address (for extra clients), and a
/// shutdown handle. The server thread is detached; `stop` ends its accept
/// loop.
struct Harness {
    conn: ClientConn,
...
/// Buffered reader/writer for one RTSP TCP connection. Holding a per-connection
/// lookahead buffer is essential: after a control response, RTP `$`-frames
/// arrive on the same socket, and a plain `read` can return them together
/// with the response. Consuming only the bytes belonging to the current
/// message leaves the rest for the next read, eliminating a lost-frame race.
struct ClientConn {
    stream: TcpStream,
    buf: Vec<u8>,
}
...
impl ClientConn {
    ...
    /// read timeout (used as a "no more data yet" signal by
...
/// present, returning it plus the number of bytes consumed (marker scan +
/// 4-byte framing header + payload).
...
fn happy_path_tcp_interleaved_serves_rtp_after_play() {
    ...
    h.conn
        .send("OPTIONS rtsp://127.0.0.1/stream RTSP/1.0\r\nCSeq: 1\r\n\r\n");
...
fn client_disconnect_shrinks_hub_client_count_to_baseline() {
    ...
    assert!(
        wait_until(|| state.client_count() == baseline + 1),
        "SETUP must register one client; count={}",
        state.client_count()
    );
    ...
    assert!(
        wait_until(|| state.client_count() == baseline),
        "client count must return to baseline after disconnect; count={}",
        state.client_count()
    );
}
...
fn two_concurrent_clients_each_receive_interleaved_rtp() {
    ...
    assert!(
        wait_until(|| state.client_count() == 2),
        "both clients must be registered; count={}",
        state.client_count()
    );
