...
/// Appends one FLV tag (11-byte header + `body` + 4-byte previous-tag-size) to `out`.
...
/// Standard-path video seq-header tag body: `0x17` (keyframe+AVC), AVCPacketType 0 (seq header), 3-byte composition time, then the config record.
...
/// Standard-path video NALU tag body: `frame_byte` (keyframe `0x17` or inter `0x27`), AVCPacketType 1 (NALU), 3-byte composition time, then length-prefixed NALUs.
...
/// Extended-path video SequenceStart tag body: ExVideoTagHeader `0x90` (ex=1, ftype=1, ptype=0) + FourCC `avc1` + config record.
...
/// Extended-path video CodedFramesX tag body: ExVideoTagHeader (keyframe `0x93` or inter `0xA3`, both ptype=3) + FourCC `avc1` + length-prefixed NALUs (no composition time).
...
/// The FLV header + leading previous-tag-size (0x00000000) that precedes the first tag, with the uPFLV prefix prepended when `with_prefix` is set. Returned separately so a multi-message test can split header from tags.
...
/// Builds a synthetic extendedFlv stream. `with_prefix` toggles the uPFLV prefix. `seq_header`/`keyframe_body`/`inter_body` are the video-tag payloads (standard or extended path); `metadata` optionally prepends an `onMetaData` script tag.
pub fn build_stream(with_prefix: bool, metadata: Option<(u32, u32, f64)>, seq_header: Vec<u8>, keyframe_body: Vec<u8>, inter_body: Vec<u8>) -> Vec<u8> {
